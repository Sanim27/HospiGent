...
# Helper function to get the next day of the week
    ...
    if connection.is_connected():
        ...
        st.session_state.messages.append({"role":"assistant","content":f"{doctor_name}'s all appointments have been scheduled for tomorrow and all the patients of {doctor_name} have been notified through email as well."})
        display_in_chunks_with_cursor(f"{doctor_name}'s all appointments have been scheduled for tomorrow and all the patients of {doctor_name} have been notified through email as well.")
...
def send_message_to_patient(patients_dict, message):
    ...
    if connection.is_connected():
        ...
        if not result:
            ...
            st.session_state.messages.append({"role":"assistant","content":"we have No any patients of such names.Can you please recheck and provide correct names of the patients."})
            cursor.close()
            connection.close()
            return
...
def get_info_client(query):
    ...
    if result:
        ...
        st.session_state.messages.append({"role":"assistant","content":"some error realted to query occured."})
